...
async function insertCategory(title, categoryAppleId) {
  return safeFetchJson(`${API_PATH}/categories`, {
    method: 'POST',
    headers: {
...
async function insertApp({ appTitle, appleId, appUrl, categoryId }) {
  const body = { title: appTitle, category_id: categoryId };
  if (appleId) body.apple_id = appleId;
  if (appUrl) body.url = appUrl;

  return safeFetchJson(`${API_PATH}/apps/node`, {
    method: 'POST',
    headers: {
      token: `token ${USER_UID}`,
      'Content-Type': 'application/json',
